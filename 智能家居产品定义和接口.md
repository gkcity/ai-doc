...
设备组，将某些编组在一起，称之为设备组。设备编组满足：

* type必须一样，版本可以不同。

所以：

* 设备组的实例定义和设备实例定义一模一样，type由device变成group。
* 设备编组后的type版本取设备里的最低版本。

如将多个灯泡编组，灯泡的type分别为：

```
urn:joy-spec:device:lightbulb:00000016:gkct:deo:1
urn:joy-spec:device:lightbulb:00000016:gkct:deo:1
urn:joy-spec:device:lightbulb:00000016:gkct:deo:2
urn:joy-spec:device:lightbulb:00000016:gkct:deo:3
urn:joy-spec:device:lightbulb:00000016:gkct:deo:1
```
...
```
urn:joy-spec:group:lightbulb:00000016:gkct:deo:1
```

## 二、设备操作

操作一个设备，需要发送HTTP消息到localhost:8080端口。
...
* 请求
    ...
    ```json
    GET /property?pid={did}.{siid}.{piid}
    ```
